...
*/
router.get("/", request => {
  const base = 'https://dentist-bot.cuki.workers.devrs.dev' + "/index.html";
...
router.get("/setup-bot", async () => {
  ...
  const req = {
    body: encodeURIComponent('url') + '=' + encodeURIComponent('https://dentist-bot.cuki.workers.devrs.dev' + '/secreat_chat_patht'),
    method: "POST",
    headers: {
  ...
  var webhook_url = 'https://dentist-bot.cuki.workers.devrs.dev' + '/secreat_chat_patht';
...
router.post("/msg", async request => {
  const formData = await request.formData();
  const body = Object.fromEntries(formData);
  var msg = body.msg.includes('photos/') ? 'https://dentist-bot.cuki.workers.devrs.dev' + '/assets/' + body.msg : body.msg;
